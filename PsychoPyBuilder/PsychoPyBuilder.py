...
class ProcedurePage(wx.ScrolledWindow):
    """A frame to represent a single procedure
    """
    def __init__(self, parent, id=-1, proc=None):
...
class ProceduresNotebook(wx.aui.AuiNotebook):
    ...
    def getCurrentProc(self):
        return self.getCurrentPage().proc
...
class ProcButtonsPanel(scrolled.ScrolledPanel):
    ...
    def onObjectAdd(self,evt):
        ...
        print newObj.params
        currProcPage = self.parent.procPanel.getCurrentPage()
        currProc = self.parent.procPanel.getCurrentProc()
        currProc.append(newObj)
        currProcPage.Refresh()
