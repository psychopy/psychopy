...
.. raw:: html
   ...
   <script>
      ...
      document.addEventListener('DOMContentLoaded', evt => {
        for (let tab of document.getElementsByClassName(tabClass)) {
          document.getElementById(tab.htmlFor).checked = true
        }
      })
   </script>
...
.. _manual_install:
.. dropdown:: Manual installations

  See below for options if you want to run the Python app but don't want to use the Standalone releases.

  .. _pip_install:
  .. dropdown:: pip install
  ...
  .. _conda:
  .. dropdown:: Anaconda and Miniconda
