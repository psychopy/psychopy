...
ioHub processes device events in parallel to the execution of the main 
experiment script. This means, for instance, that keyboard and mouse events
are processed and time stamped quickly, regardless of whether the experiment 
script is in a blocking state when the event is received. A PsychoPy experiment 
blocks, for example, from when :func:`psychopy.visual.window.flip()`
