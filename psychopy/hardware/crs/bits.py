...
class BitsBox:
    ...
    def _prepareFBOrender(self):
        ...
        if self.mode.startswith('mono'):
            gl.glUseProgram(shaders.bitsMonoModeFrag)
        elif self.mode.startswith('colo'):
            gl.glUseProgram(shaders.bitsColorModeFrag)
    def _finishFBOrender(self):
        """Unload the shader program, as needed. If we're in mono++ mode then drawLUT
        """
    ...
    def reset(self):
        reset()
    def __del__(self):
        print 'psychopy.hardware.crs: cleaning up BitsBox'
