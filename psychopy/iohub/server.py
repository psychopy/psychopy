...
class udpServer(DatagramServer):
    ...
    def handleExperimentDeviceRequest(self, request, replyTo):
        ...
        if request_type == 'EVENT_TX':
            ...
            for eventAsTuple in exp_events:
                ioServer.deviceDict[
                    'Experiment']._nativeEventCallback(eventAsTuple)
            ...
            data = self.iohub.createNewMonitoredDevice(
                dclass_name, dconfig_dict)
    ...
    def sendResponse(self, data, address):
        ...
        try:
            ...
            if packet_data_length >= max_size:
                ...
                self.sendResponse(
                    ('IOHUB_MULTIPACKET_RESPONSE', num_packets), address)
                ...
                self.socket.sendto(
                    packet_data[
                        (p + 1) * max_size:packet_data_length],
                    address)
            else:
    ...
    def setExperimentInfo(self, experimentInfoList):
        ...
        if self.iohub.emrt_file:
            exp_id = self.iohub.emrt_file.createOrUpdateExperimentEntry(
                experimentInfoList)
    ...
    def checkIfSessionCodeExists(self, sessionCode):
        try:
            if self.iohub.emrt_file:
                return self.iohub.emrt_file.checkIfSessionCodeExists(
                    sessionCode)
            return False
        except Exception:
            printExceptionDetailsToStdErr()
    ...
    def initializeConditionVariableTable(
            self, experiment_id, session_id, numpy_dtype):
        if self.iohub.emrt_file:
            output = []
            ...
            return self.iohub.emrt_file._initializeConditionVariableTable(
                experiment_id, session_id, output)
        return False

    def addRowToConditionVariableTable(self, experiment_id, session_id, data):
        if self.iohub.emrt_file:
            return self.iohub.emrt_file._addRowToConditionVariableTable(
                experiment_id, session_id, data)
        return False
...
class ioServer(object):
            ...
            if 'data_store' in config and _DATA_STORE_AVAILABLE:
                ...
                if experiment_datastore_config.get('enable', True):
                    ...
                    self.createDataStoreFile(
                        experiment_datastore_config.get(
                            'filename',
                            'events') + '.hdf5',
                        resultsFilePath,
                        'a',
    ...
    def processDeviceConfigDictionary(
            self,
            device_module_path,
            device_class_name,
            device_config_dict,
            default_device_config_dict):
        for default_config_param, default_config_value in default_device_config_dict.iteritems():
            if default_config_param not in device_config_dict:
                if isinstance(default_config_value, (dict, OrderedDict)):
                    ...
                    self.processDeviceConfigDictionary(
                        None, None, device_param_value, default_config_value)
                    device_config_dict[
                        default_config_param] = device_param_value
                else:
                    device_config_dict[
                        default_config_param] = default_config_value
        ...
        if device_module_path and device_class_name:
            ...
            device_config_errors = validateDeviceConfiguration(
                device_module_path, device_class_name, device_config_dict)

            for error_type, error_list in device_config_errors.iteritems():
                if len(error_list) > 0:
                    device_errors = self._all_device_config_errors.get(
                        device_module_path, {})
                    device_errors[error_type] = error_list
                    self._all_device_config_errors[
                        device_module_path] = device_errors
        ...
        if device_class_name in ('Mouse', 'Keyboard'):
            if Computer.system == 'win32':
                ...
                if device_class_name == 'Mouse' and 'Mouse' not in self._hookDevice:
                    ...
                    mouseHookMonitor = DeviceMonitor(
                        deviceDict['Mouse'], 0.004)
                ...
                if device_class_name == 'Keyboard' and 'Keyboard' not in self._hookDevice:
                    ...
                    kbHookMonitor = DeviceMonitor(
                        deviceDict['Keyboard'], 0.004)
        ...
        if device_config.get('enable', True):
            ...
            if isinstance(monitor_events_list, (list, tuple)):
                for event_class_name in monitor_events_list:
                    event_id = getattr(
                        EventConstants, convertCamelToSnake(
                            event_class_name[
                                :-5], False))
    ...
    def createDataStoreFile(self, fileName, folderPath, fmode, ioHubsettings):
        if _DATA_STORE_AVAILABLE:
            ...
            self.emrt_file = ioHubpyTablesFile(
                fileName, folderPath, fmode, ioHubsettings)
