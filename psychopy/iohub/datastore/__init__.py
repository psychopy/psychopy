...
class ioHubpyTablesFile():
            ...
            if event_cls.IOHUB_DATA_TABLE:
                ...
                if event_table_label not in self.TABLES.keys():
                    try:
                        ...
                        # self.emrtFile.createTable(,eventTableLabel2ClassName(event_table_label),event_cls.NUMPY_DTYPE,
                        # title="%s
                        # Data"%(device_instance.__class__.__name__,),filters=dfilter.copy())
                        self.TABLES[event_table_label] = self._eventGroupMappings[
                            event_table_label]._f_get_child(eventTableLabel2ClassName(event_table_label))
    ...
    def loadTableMappings(self):
        ...
        try:
            ...
            # Just means the table for this event type has not been created as
            # the event type is not being recorded
            pass
        ...
        try:
            self.TABLES[
                'WINTAB_TABLET_SAMPLE'] = self.emrtFile.root.data_collection.events.wintab.WintabTabletSampleEvent
        except:
            # Just means the table for this event type has not been created as
            # the event type is not being recorded
            pass
        try:
            self.TABLES[
                'WINTAB_TABLET_LEAVE_REGION'] = self.emrtFile.root.data_collection.events.wintab.WintabTabletLeaveRegionEvent
        except:
            # Just means the table for this event type has not been created as
            # the event type is not being recorded
            pass
        try:
            self.TABLES[
                'WINTAB_TABLET_ENTER_REGION'] = self.emrtFile.root.data_collection.events.wintab.WintabTabletEnterRegionEvent
        except:
            # Just means the table for this event type has not been created as
            # the event type is not being recorded
            pass
        ...
        try:
            self.TABLES[
                'ANALOG_INPUT'] = self.emrtFile.root.data_collection.events.mcu.AnalogInputEvent
        except Exception:
            # Just means the table for this event type has not been created as
            # the event type is not being recorded
            pass
        ...
        try:
            self.TABLES[
                'THRESHOLD'] = self.emrtFile.root.data_collection.events.mcu.ThresholdEvent
        except Exception:
            # Just means the table for this event type has not been created as
            # the event type is not being recorded
            pass
        ...
        try:
            self.TABLES[
                'DIGITAL_INPUT'] = self.emrtFile.root.data_collection.events.mcu.DigitalInputEvent
        except Exception:
            # Just means the table for this event type has not been created as
            # the event type is not being recorded
            pass
        ...
        try:
            self.TABLES[
                'SERIAL_INPUT'] = self.emrtFile.root.data_collection.events.serial.SerialInputEvent
        except Exception:
            # Just means the table for this event type has not been created as
            # the event type is not being recorded
            pass
        ...
        try:
            self.TABLES[
                'SERIAL_BYTE_CHANGE'] = self.emrtFile.root.data_collection.events.serial.SerialByteChangeEvent
        except Exception:
            # Just means the table for this event type has not been created as
            # the event type is not being recorded
            pass
        ...
        try:
            self.TABLES[
                'PSTBOX_BUTTON'] = self.emrtFile.root.data_collection.events.serial.PstBoxButtonEvent
        except Exception:
            # Just means the table for this event type has not been created as
            # the event type is not being recorded
            pass
        ...
        try:
            self.TABLES[
                'EYE_SAMPLE'] = self.emrtFile.root.data_collection.events.eyetracker.EyeSampleEvent
        except Exception:
            # Just means the table for this event type has not been created as
            # the event type is not being recorded
            pass
    ...
    def buildOutTemplate(self):
        ...
        self.emrtFile.createGroup(
            self.emrtFile.root.data_collection.events,
            'experiment',
        ...
        self.emrtFile.createGroup(
            self.emrtFile.root.data_collection.events,
            'wintab',
            title='Wintab Tablet Device Events.')
        self.emrtFile.createGroup(
            self.emrtFile.root.data_collection.events,
            'gamepad',
        ...
        self.emrtFile.createGroup(
            self.emrtFile.root.data_collection.events,
            'mcu',
            title='MCU Device Events.')
        self.emrtFile.createGroup(
            self.emrtFile.root.data_collection.events,
            'serial',
            title='Serial Interface Events.')
        self.flush()
    ...
    def _buildEventGroupMappingDict(self):
        ...
        self._eventGroupMappings[
            'WINTAB_TABLET_SAMPLE'] = self.emrtFile.root.data_collection.events.wintab
        self._eventGroupMappings[
            'WINTAB_TABLET_ENTER_REGION'] = self.emrtFile.root.data_collection.events.wintab
        self._eventGroupMappings[
            'WINTAB_TABLET_LEAVE_REGION'] = self.emrtFile.root.data_collection.events.wintab
        self._eventGroupMappings[
        ...
        self._eventGroupMappings[
            'ANALOG_INPUT'] = self.emrtFile.root.data_collection.events.mcu
        self._eventGroupMappings[
            'THRESHOLD'] = self.emrtFile.root.data_collection.events.mcu
        self._eventGroupMappings[
            'DIGITAL_INPUT'] = self.emrtFile.root.data_collection.events.mcu
        self._eventGroupMappings[
            'SERIAL_INPUT'] = self.emrtFile.root.data_collection.events.serial
        self._eventGroupMappings[
            'SERIAL_BYTE_CHANGE'] = self.emrtFile.root.data_collection.events.serial
        self._eventGroupMappings[
            'PSTBOX_BUTTON'] = self.emrtFile.root.data_collection.events.serial
        self._eventGroupMappings[
            'MESSAGE'] = self.emrtFile.root.data_collection.events.experiment
        self._eventGroupMappings[
            'LOG'] = self.emrtFile.root.data_collection.events.experiment
        self._eventGroupMappings[
            'EYE_SAMPLE'] = self.emrtFile.root.data_collection.events.eyetracker
        self._eventGroupMappings[
...
#            print2err("*** ",DeviceEvent.EVENT_TYPE_ID_INDEX, '_handleEvent: ',etype,' : event list: ',event)
    ...
    def _handleEvents(self, events):
        ...
        try:
            ...
            for event in events:
                event[
                    DeviceEvent.EVENT_EXPERIMENT_ID_INDEX] = self.active_experiment_id
                event[DeviceEvent.EVENT_SESSION_ID_INDEX] = self.active_session_id
