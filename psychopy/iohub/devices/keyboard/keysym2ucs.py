...
# Converted to python by Sol Simpson, April 8th, 2013
#
# Each inner tuple is a (keysym,unicode) pair.
...
def keysym2ucs(keysym):
    ...
    while max >= min:
        mid = (min + max) / 2
        if mid >=len(keysymtab):
            break
        if keysymtab[mid][0] < keysym:
            min = mid + 1
        elif keysymtab[mid][0] > keysym:
            max = mid - 1
        else:
            # found it 
            return keysymtab[mid][1]
