...
"""ioHub Eye tracker interface code for Tobii EyeX devices.
...
class TobiiEyeXTracker():
    ...
    def __init__(self):
        ...
        if WINDOWS_64BIT:
            print2err(
                'Although the Tobii 64-bit driver should work in theory, it is untested inside psychopy')
            self.dll = WinDLL(
        ...
        # NOTE: First arg is return type, was c_int but the C example returns
        # void
        self.gaze_callback_type = callback_generator(None, POINTER(
            TobiiGazeData), POINTER(TobiiGazeDataExtensions), c_void_p)
        ...
        self.dll.tobiigaze_get_connected_eye_tracker.argtypes = [
            c_char_p, c_uint32, POINTER(c_uint32)]
        self.dll.tobiigaze_get_connected_eye_tracker(
            self.url, c_url_size, None)

        self.eye_tracker = c_void_p(self.dll.tobiigaze_create(self.url, None))
        self.info = TobiiDeviceInfo()
        self.dll.tobiigaze_run_event_loop_on_internal_thread(
            self.eye_tracker, None, None)
        ...
        self.dll.tobiigaze_get_device_info(
            self.eye_tracker, byref(
                self.info), byref(
                self.error_code))
    ...
    def get_gaze_callback(self):
        def func(
                tobiigaze_gaze_data_ref,
                tobiigaze_gaze_data_extensions_ref,
                user_data):
    ...
    def startTracking(self, python_function):
        ...
        self.dll.tobiigaze_start_tracking(
            self.eye_tracker, self.gaze, byref(
        ...
        if DEBUG:
            print(
                'Start tracking finished, status: %s' %
                self.error_code.value)

    def stopTracking(self):
        self.dll.tobiigaze_stop_tracking(
            self.eye_tracker, byref(self.error_code))
    ...
    def getDelayInMicroseconds(self, timestamp, convert_to_sec):
        """The Tobii EyeX timestamp starts at a random number of microseconds,
        so we need to track when we started.
    ...
    def WrapAsyncCallback(self, callback, name):
        def callback_wrapper(error, data):
            ...
            if error > 0:
                print2err(
                    'Got calibration {0} callback error: {1} with data {2}'.format(
                        name, error, data))
    ...
    def StartCalibration(self, callback):
        ...
        self.dll.tobiigaze_calibration_start_async(
            self.eye_tracker, nativeCallback, None)

    def StopCalibration(self, callback):
        nativeCallback = self.WrapAsyncCallback(callback, 'stop')
        self.dll.tobiigaze_calibration_stop_async(
            self.eye_tracker, nativeCallback, None)
    ...
    def AddCalibrationPoint(self, p, callback):
        # This has to take a calibration point from the other Tobii API and put
        # it into this one
        ...
        self.dll.tobiigaze_calibration_add_point_async(
            self.eye_tracker, addressof(point), nativeCallback, None)
    ...
    def RemoveCalibrationPoint(self, p, callback):
        ...
        self.dll.tobiigaze_calibration_remove_point_async(
            self.eye_tracker, addressof(point), nativeCallback, None)
    ...
    def ComputeCalibration(self, callback):
        ...
        self.dll.tobiigaze_calibration_compute_and_set_async(
            self.eye_tracker, nativeCallback, None)
