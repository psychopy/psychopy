...
class QueryPacket(SmartSetPacket):
    ...
    def __init__(self, b2=0, b3=0, b4=0, b5=0, b6=0, b7=0, b8=0):
        SmartSetPacket.__init__(self, getTime(), bytearray(
            [self.LEAD_IN_BYTE, self.PACKET_TYPE_CHAR, b2, b3, b4, b5, b6, b7, b8, 0]))
...
class ResponsePacket(SmartSetPacket):
    ...
    def __init__(self, time, _packet_bytes):
        ...
        if len(_packet_bytes) != 10:
            ...
            print2err(
                'Warning: ResponsePacket _packet_bytes must be 10 bytes in length: %s' % str(
                    self._packet_bytes))
        if _packet_bytes[1] != ord(self.PACKET_TYPE_CHAR):
            self.valid_respons = False
            print2err(
                'Warning: ResponsePacket PACKET_TYPE_CHAR must equal %s: %s' %
                (self.PACKET_TYPE_CHAR, str(
                    self._packet_bytes)))
...
RESPONSE_PACKET_TYPES[
    ResponseAcknowledge.PACKET_TYPE_CHAR] = ResponseAcknowledge
...
class QueryCalibration(QueryPacket):
    ...
    def __init__(self, axis=0, swap_flag=False):
        if axis and swap_flag is True:
            print2err(
                'Warning: QueryCalibration axis and swap_flag args can not be used at the same time.')
        if axis:
...
class CommandCalibration(CommandPacket):
    ...
    def __init__(self, axis=0, low_point=0, high_point=0,
        ...
        if axis and enable_swap is True:
            print2err(
                'Warning: CommandCalibration axis and enable_swap args can not be used at the same time.')
        if axis:
            if low_point is not None and high_point is not None:
                # get first byte of lowpoint 2-bytes and convert to chr
                low_point_chr1 = int(low_point) & 0b11111111
                # get second byte of lowpoint 2-bytes and convert to chr
                low_point_chr2 = int(low_point) >> 8
                # get first byte of highpoint 2-bytes and convert to chr
                high_point_chr1 = int(high_point) & 0b11111111
                # get second byte of highpoint 2-bytes and convert to chr
                high_point_chr2 = int(high_point) >> 8
                ...
                # get first byte of offset 2-bytes and convert to chr
                offset_chr1 = int(offset) & 0b11111111
                # get second byte of offset 2-bytes and convert to chr
                offset_chr2 = int(offset) >> 8
                # get first byte of numerator 2-bytes and convert to chr
                numerator_chr1 = int(numerator) & 0b11111111
                # get second byte of numerator 2-bytes and convert to chr
                numerator_chr2 = int(numerator) >> 8
                # get first byte of denominator 2-bytes and convert to chr
                denominator_chr1 = int(denominator) & 0b11111111
                # get second byte of denominator 2-bytes and convert to chr
                denominator_chr2 = int(denominator) >> 8
                CommandPacket.__init__(
                    self,
                    axis.lower(),
                    offset_chr1,
...
class ResponseCalibration(ResponsePacket):
    ...
    def __init__(self, time, packet_bytes):
        ...
        self.numerator = int(
            self._packet_bytes[6] << 8 +
            self._packet_bytes[5])
        self.denominator = int(
            self._packet_bytes[8] << 8 +
            self._packet_bytes[7])
RESPONSE_PACKET_TYPES[
    ResponseCalibration.PACKET_TYPE_CHAR] = ResponseCalibration
...
class CommandDiagnostics(CommandPacket):
    ...
    def __init__(
        ...
        diag_mask = self.id_test + self.cpu_test + self.rom_test + \
            self.ram_test + self.nvram_test + self.drive_test + self.chop_test
...
RESPONSE_PACKET_TYPES[
    ResponseDiagnostics.PACKET_TYPE_CHAR] = ResponseDiagnostics
...
class CommandFilter(CommandPacket):
    ...
    def __init__(self, sample_avg_count=4, sample_dev_thresh=8,
        ...
        CommandPacket.__init__(
            self,
            '0',
            chr(sample_avg_count),
            chr(sample_dev_thresh),
            chr(samples_before_state_change),
            chr(state))
...
class ResponseFilter(ResponsePacket):
    ...
    def __init__(self, time, packet_bytes):
        ...
        self.drive_sig_change_delay = (
            (self._packet_bytes[6] & 0b00001111) + 1) * .5
...
class CommandTimer(CommandPacket):
    ...
    **NOTE:**
    ...
    def __init__(self, enable=1, mode=0, interval=1000):
        ...
        CommandPacket.__init__(self, chr(enable), chr(mode), chr(
            0), chr(interval_low_byte), chr(interval_high_byte))
...
class ResponseID(ResponsePacket):
    ...
    def __init__(self, time, packet_bytes):
        ResponsePacket.__init__(self, time, packet_bytes)
        self.screen_type = self.typeChar2ModelType.get(
            chr(self._packet_bytes[2]), 'UNKNOWN')
        self.comm_interface = self.ioChar2CommType.get(
            chr(self._packet_bytes[3]), 'UNKNOWN')
        ...
        self.controller_model = self.ctrlByte2CtrlType.get(
            int(self._packet_bytes[8]), 'UNKNOWN')
...
class CommandParameter(CommandPacket):
    ...
    def __init__(
        ...
        CommandPacket.__init__(
            self,
            self.io_type,
            self.serial_settings1,
            self.serial_settings2)
