...
print("TODO: Test with switch to use lauchHubServer.")
...
if __name__ == "__main__":
    ...
    io = launchHubServer(experiment_code='gc_cursor', iohub_config_name="iohub_config.yaml")
    ...
    io.sendMessageEvent(text="Stimulus Screen ID: {0}, Size (pixels): {1}, CoordType: {2}".format(display.getIndex(),
    ...
    for trial in trials:
        ...
        while run_trial is True:
            ...
            #
            if type(gpos) in [tuple,list]:
                io.sendMessageEvent("IMAGE_UPDATE %s %.3f %.3f"%(
                                            trial['IMAGE_NAME'],gpos[0],gpos[1]),
                                            sec_time=flip_time)
            else:
                io.sendMessageEvent("IMAGE_UPDATE %s [NO GAZE]"%(
                                            trial['IMAGE_NAME']),
                                            sec_time=flip_time)
    ...
    flip_time=window.flip()
    io.sendMessageEvent(text='EXPERIMENT_COMPLETE',sec_time=flip_time)
