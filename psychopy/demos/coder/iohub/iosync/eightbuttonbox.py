...
class ButtonBoxState(object):
    ...
    @property
    def pressed(self):
        # [bname for bname, mask in self.masks.items() if self._state & mask]
        return self._pressed.items()

    @property
    def released(self):
        # [bname for bname, mask in self.masks.items() if self._state & mask]
        return self._released.items()
    ...
    def setDigitalInputEvent(self, din_event):
        ...
        new_pressed = [
            bname for bname,
            mask in self.masks.items() if self._state & mask]
