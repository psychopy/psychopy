...
if psychopy.iohub._DATA_STORE_AVAILABLE is False:
    raise ImportError(
        'DataStore module could not be imported. (Likely that pyTables hdf5dll could not be found). Exiting demo...')
    sys.exit(1)
...
def writeDataRow(output_file, session_info, session_uservar_names, event_data):
    """Save a row of data to the output file, in tab delimited format.

    See comment for writeOutputFileHeader function for order of saved
    columns.

    """
    session_data = [str(i) for i in session_info[:-1]]
    session_user_data = [session_info.user_variables[sud_name]
                         for sud_name in session_uservar_names]
    all_data = session_data + session_user_data + \
        [str(e) for e in event_data] + session_user_data
...
if __name__ == '__main__':
    ...
    data_file_path = displayDataFileSelectionDialog(
        psychopy.iohub.util.module_directory(writeOutputFileHeader))
    ...
    dataAccessUtil = ExperimentDataAccessUtility(
        dpath, dfile, experimentCode=None, sessionCodes=[])
    ...
    with open(log_file_name, 'w') as output_file:
        ...
        writeOutputFileHeader(
            output_file,
            session_metadata_columns,
            dataAccessUtil.getEventTable(event_class_selection).cols._v_colnames[
                3:])
    ...
    print(
        '%s will be in the same directory as the selected .hdf5 file' %
        (log_file_name))
