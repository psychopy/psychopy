...
"""EloTouchScreenDemo illustrates how to use the Elo Serial Touch Screen
    ...
    Once the calibration / validation stages have been performed, the
    demo draws a circle stimulus where ever the touch screen is pressed.
...
def getTouchPoint(x, y):
    ...
    demo_stim['touch_point_stim'].pos = (x, y)
    demo_stim['touch_point_stim'].draw()
...
def run_elo_calibration():
    ...
    window.flip()
    io.clearEvents()
    while not io.devices.keyboard.getPresses():
        time.sleep(0.05)
    io.clearEvents()
...
def run_elo_validation():
    ...
    demo_stim['cal_instruct_stim'].setText(txt)
    demo_stim['cal_instruct_stim'].setPos((0, 0))
    demo_stim['cal_instruct_stim'].draw()
    window.flip()
    io.clearEvents()
...
if __name__ == '__main__':
    ...
    #   Report
    #
    hw_conf_dict = io.devices.touch.getHardwareConfiguration()
    import pprint
    print('hw_conf_dict:')
    ...
    if calibrate_elo is True:
        # Calibrate
        run_elo_calibration()
        io.clearEvents()
        # Validate
        terminate_calibration = False
        while not terminate_calibration and run_elo_validation() is False:
            run_elo_calibration()
            if io.devices.keyboard.getEvents():
        ...
        io.devices.touch.saveConfiguration()
    else:
        io.devices.touch.restoreConfiguration()
    ...
    while run_demo:
        ...
        if touch_events:
            ...
            demo_stim['touch_contingent_stim'].draw()
            window.flip()
        if io.devices.keyboard.getPresses():
            run_demo = False
