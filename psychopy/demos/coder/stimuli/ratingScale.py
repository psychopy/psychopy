...
visual.InstructionScreen(myWin, """This is a demo of visual.RatingScale(). There are three examples.
...
visual.InstructionScreen(myWin, """Example 2. This example uses non-default settings for the visual display, skipping a rating is not possible, and it uses a list of images (two) to be rated on several dimensions (valence and arousal).
...
visual.InstructionScreen(myWin, """Example 3. This example shows how one could obtain two ratings at the same time, e.g., to allow explicit comparison between images during ratings.
...
visual.InstructionScreen(myWin, """Example 4.
