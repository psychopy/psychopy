...
class PreferencesDlg(wx.Dialog):
    ...
    def __init__(self, app,
        ...
        wx.Dialog.__init__(
            self, None, -1, _translate("PsychoPy Preferences"), pos, size, style)
        self.app = app
        self.Center()
        ...
        self.ctrls = {}
        sectionOrdering = ['app', 'builder', 'coder',
                           'general', 'connections', 'keyBindings']
        for sectionName in sectionOrdering:
            prefsPage = self.makePrefsPage(parent=self.nb,
                                           sectionName=sectionName,
                                           prefsSection=self.prefsCfg[
                                               sectionName],
        ...
        btn.SetHelpText(_translate(
            "Save prefs (in all sections) and close window"))
        ...
        btn.SetHelpText(_translate(
            "Apply these prefs (in all sections) and continue"))
    ...
    def setPrefsFromCtrls(self):
        ...
        for sectionName in self.prefsCfg.keys():
            for prefName in self.prefsSpec[sectionName].keys():
                ...
                # make sure list values are converted back to being lists (from
                # strings)
                if self.prefsSpec[sectionName][prefName].startswith('list'):
                    try:
                        ...
                        warnDlg = dialogs.MessageDialog(parent=self,
                                                        message=_translate('Invalid value in "%(pref)s" ("%(section)s" Tab)') % {
                                                            'pref': pLabel, 'section': sLabel},
...
class PrefCtrls(object):
    ...
    def _getCtrlValue(self, ctrl):
        ...
        if ctrl is None:
            ...
            logging.warning(
                "failed to retrieve the value for pref: %s" % (ctrl.valueCtrl))
            return None
