...
class BuilderFrame(wx.Frame):
    ...
    def makeToolbar(self):
        ...
        if platform.system() == 'Darwin':
            self.toolbar.AddSimpleTool(self.IDs.tbFileNew, new_bmp, ("New [%s]" %self.app.keys.new).replace('Ctrl+', 'Cmd+'), "Create new python file")
            self.toolbar.Bind(wx.EVT_TOOL, self.fileNew, id=self.IDs.tbFileNew)
            self.toolbar.AddSimpleTool(self.IDs.tbFileOpen, open_bmp, ("Open [%s]" %self.app.keys.open).replace('Ctrl+', 'Cmd+'), "Open an existing file'")
            self.toolbar.Bind(wx.EVT_TOOL, self.fileOpen, id=self.IDs.tbFileOpen)
            self.toolbar.AddSimpleTool(self.IDs.tbFileSave, save_bmp, ("Save [%s]" %self.app.keys.save).replace('Ctrl+', 'Cmd+'),  "Save current file")
            self.toolbar.EnableTool(self.IDs.tbFileSave, False)
            self.toolbar.Bind(wx.EVT_TOOL, self.fileSave, id=self.IDs.tbFileSave)
            self.toolbar.AddSimpleTool(self.IDs.tbFileSaveAs, saveAs_bmp, ("Save As... [%s]" %self.app.keys.saveAs).replace('Ctrl+', 'Cmd+'), "Save current python file as...")
            self.toolbar.Bind(wx.EVT_TOOL, self.fileSaveAs, id=self.IDs.tbFileSaveAs)
            self.toolbar.AddSimpleTool(self.IDs.tbUndo, undo_bmp, ("Undo [%s]" %self.app.keys.undo).replace('Ctrl+', 'Cmd+'), "Undo last action")
            self.toolbar.Bind(wx.EVT_TOOL, self.undo, id=self.IDs.tbUndo)
            self.toolbar.AddSimpleTool(self.IDs.tbRedo, redo_bmp, ("Redo [%s]" %self.app.keys.redo).replace('Ctrl+', 'Cmd+'),  "Redo last action")
            self.toolbar.Bind(wx.EVT_TOOL, self.redo, id=self.IDs.tbRedo)
            self.toolbar.AddSeparator()
            self.toolbar.AddSeparator()
            self.toolbar.AddSimpleTool(self.IDs.tbPreferences, preferences_bmp, "Preferences [Cmd+,]",  "Application preferences")
            ...
            self.toolbar.AddSimpleTool(self.IDs.tbCompile, compile_bmp, ("Compile Script [%s]" %self.app.keys.compileScript).replace('Ctrl+', 'Cmd+'),  "Compile to script")
            self.toolbar.Bind(wx.EVT_TOOL, self.compileScript, id=self.IDs.tbCompile)
            self.toolbar.AddSimpleTool(self.IDs.tbRun, run_bmp, ("Run [%s]" %self.app.keys.runScript).replace('Ctrl+', 'Cmd+'),  "Run experiment")
            self.toolbar.Bind(wx.EVT_TOOL, self.runFile, id=self.IDs.tbRun)
            self.toolbar.AddSimpleTool(self.IDs.tbStop, stop_bmp, ("Stop [%s]" %self.app.keys.stopScript).replace('Ctrl+', 'Cmd+'),  "Stop experiment")
            ...
            self.toolbar.Bind(wx.EVT_TOOL, self.compileScript, id=self.IDs.tbCompile)
            self.toolbar.AddSimpleTool(self.IDs.tbRun, run_bmp, "Run [%s]" %self.app.keys.runScript,  "Run experiment")
            self.toolbar.Bind(wx.EVT_TOOL, self.runFile, id=self.IDs.tbRun)
            self.toolbar.AddSimpleTool(self.IDs.tbStop, stop_bmp, "Stop [%s]" %self.app.keys.stopScript,  "Stop experiment")
            self.toolbar.Bind(wx.EVT_TOOL, self.stopFile, id=self.IDs.tbStop)
            self.toolbar.EnableTool(self.IDs.tbStop,False)
