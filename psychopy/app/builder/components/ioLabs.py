...
tooltip = _translate(
    'ioLabs ButtonBox: check and record response buttons on ioLab Systems ButtonBox')
...
class ioLabsButtonBoxComponent(KeyboardComponent):
    ...
    def __init__(self, exp, parentName, name='bbox',
        ...
        self.params['correctAns'].hint = _translate(
            "What is the 'correct' response? NB, buttons are labelled 0 to 7 on a 8-button box. Enter 'None' (no quotes) if withholding a response is correct. Might be helpful to add a correctAns column and use $correctAns to compare to the key press.")
        self.params['store'].allowedVals = [
            'last button', 'first button', 'all buttons', 'nothing']
        self.params['store'].hint = _translate(
    ...
    def writeFrameCode(self, buff):
        ...
        if store == 'first button':
            buff.writeIndented(
                "if %(name)s.btns == []:  # True if the first\n" % self.params)
            buff.setIndentLevel(1, True)
            dedentAtEnd += 1
            buff.writeIndented(
                "%(name)s.btns = theseButtons[0].key  # just the first button\n" % self.params)
            buff.writeIndented(
                "%(name)s.rt = theseButtons[0].rt\n" % (self.params))
        elif store == 'last button':
            buff.writeIndented(
                "%(name)s.btns = theseButtons[-1].key  # just the last button\n" % self.params)
            buff.writeIndented(
                "%(name)s.rt = theseButtons[-1].rt\n" % self.params)
        elif store == 'all buttons':
            buff.writeIndented(
                "%(name)s.btns.extend([evt.key for evt in theseButtons])  # all buttons\n" % self.params)
            buff.writeIndented(
    ...
    def writeRoutineEndCode(self, buff):
        ...
        if currLoop.type == 'StairHandler':
            ...
            buff.writeIndented(
                "%s.addData('%s.btns', %s.btns)\n" % loopnamename)
            if self.params['storeCorrect'].val:
                buff.writeIndented(
                    "%s.addData('%s.corr', %s.corr)\n" % loopnamename)
            buff.writeIndented(
                "if %(name)s.btns != None:  # add RTs if there are responses\n" % self.params)
            buff.writeIndented(
                "    %s.addData('%s.rt', %s.rt)\n" % loopnamename)
