...
class ConfigWizard(object):
    ...
    def __init__(self, app, firstrun=False):
        ...
        if firstrun:
            ...
            dlg.addText("This configuration wizard will help you get going quickly and smoothly.")
        else:
        ...
        if not fatalItemsList:
            dlg.addText("We'll go through a series of checks and show the results in a browser. ")
            dlg.addText('')
            if firstrun:  # explain things more
                dlg.addText('Note: The display will switch to full-screen mode for about 10 seconds, ')
                dlg.addText("and will then switch back. You don't need to do anything.")
        ...
        dlg.addText(msg)
        for item in self.summary(items=itemsList):
            dlg.addText(item[0], item[1])
        dlg.addText('')
    ...
    def runDiagnostics(self):
        ...
        if freeRAM == 'unknown':
            ...
            report.append(('free RAM', 'unknown', msg))
        else:
            ...
            report.append(('free RAM', str(freeRAM)+'M', msg))
        ...
        msg = ''
        if items['pythonVersion'] < '2.6' or items['pythonVersion'] >= '3':
        ...
        if items['windowRefreshTimeMedian_ms'] < 3.3333333: 
            msg = """Warning: too fast? visual sync'ing with the monitor seems unlikely"""
        report.append(('refresh rate', "%.2f ms/frame" % items['windowRefreshTimeMedian_ms'], msg))
        msg = 'SD < 0.5 ms is ideal'
        ...
        #msg = 'for LCD shutter glasses, etc'
        #report.append(('openGL gl stereo', str(items['openGLext.GL_STEREO']), msg))
        win.close()
        ...
        msg = 'for online help, usage stats, updates, and google-speech'
        ...
        report.append(('proxy setting', str(self.prefs.connections['proxy']), 'current proxy setting from <a href="http://www.psychopy.org/general/prefs.html#connection-settings">Preferences -> Connections</a>'))
        ...
        if 'systemUserProcFlagged' in items:
            ...
            report.append(('background procs', self.badBgProc[0]+' ...', msg))
    ...
    def summary(self, items=None):
        ...
        green = '#009933'
        red = '#CC3300'
        summary = [('video card drivers', green)]
        ofInterest = ['python', 'free RAM', 
            'openGL version', 'refresh rate', 'refresh SD', 'no dropped frames',
            'microphone latency', 'speakers latency',
            'internet access', 'background procs']
    ...
    def htmlReport(self, items=None, fatal=False):
        ...
        if fatal:
            ...
            htmlDoc += '<p>%s (v%s) configuration report run on %s (y-m-d).</p>\n' % (self.appName, __version__, data.getDateStr(format="%Y-%m-%d"))
            numWarn = len(self.warnings)
            if numWarn == 0:
