...
class CoderFrame(wx.Frame):
    ...
    def makeToolbar(self):
        ...
        if platform.system() == 'Darwin':
            ...
            self.toolbar.AddSeparator()
            self.toolbar.AddSeparator()
            self.toolbar.AddSimpleTool(self.IDs.tbRun, run_bmp, ("Run [%s]" % self.app.keys.runScript).replace('Ctrl+', 'Cmd+'),  "Run current script")
            self.toolbar.Bind(wx.EVT_TOOL, self.runFile, id=self.IDs.tbRun)
            self.toolbar.AddSimpleTool(self.IDs.tbStop, stop_bmp, ("Stop [%s]" % self.app.keys.stopScript).replace('Ctrl+', 'Cmd+'),  "Stop current script")
            ...
            self.toolbar.AddSeparator()
            self.toolbar.AddSeparator()
            self.toolbar.AddSimpleTool(self.IDs.tbRun, run_bmp, "Run [%s]" % self.app.keys.runScript,  "Run current script")
            self.toolbar.Bind(wx.EVT_TOOL, self.runFile, id=self.IDs.tbRun)
            self.toolbar.AddSimpleTool(self.IDs.tbStop, stop_bmp, "Stop [%s]" % self.app.keys.stopScript,  "Stop current script")
            self.toolbar.Bind(wx.EVT_TOOL, self.stopFile, id=self.IDs.tbStop)
            self.toolbar.EnableTool(self.IDs.tbStop,False)
