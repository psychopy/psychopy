...
class pythonTransformer(ast.NodeTransformer):
	...
	directMathOperations = ['abs', 'min', 'max', 'round', 'ceil', 'fabs', 'floor', 'trunc', 'exp', 'log', 'log2', 'pow', 'sqrt', 'acos', 'asin', 'atan2', 'cos', 'sin', 'tan', 'acosh', 'asinh', 'atanh', 'cosh', 'sinh', 'tanh']
	...
	def visit_FormattedValue(self, node):
		...
		# formatted f-strings:
		if isinstance(node.format_spec, ast.JoinedStr) and len(node.format_spec.values) > 0 and isinstance(node.format_spec.values[0], ast.Str):
	...
	def visit_Call(self, node):
		...
		# string.format(args):
		elif isinstance(node.func, ast.Attribute) and isinstance(node.func.value, ast.Str) and node.func.attr == 'format':
...
def translatePythonToJavaScript(psychoPyCode):
	...
	try:
		...
		raise Exception('unable to translate the transformed PsychoPy code into PsychoJS JavaScript code: ' + str(error))
