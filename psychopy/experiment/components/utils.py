def loadPluginComponents():
    """
    Load relevant defs for any plugins with entry points in psychopy.experiment.components, so that they
    ...
    for name, ep in _installed_plugins_.items():
        # get points for components folder
        points = ep.get('psychopy.experiment.components', {})
