...
class NoiseStim(GratingStim):
    ...
    **Example**::
    ...
    noise1 = noise = visual.NoiseStim(
                                    ...
                                    texRes=512, filter='False', imageComponent='Phase'
    ...
    **Notes on orientation and phase**
    ...
    def _isotropic(self, FT):
        ...
        tempTex = numpy.zeros(int(self._size**2))
        tempTex = numpy.reshape(tempTex,(int(self._size),int(self._size)))
        pin = filters.makeRadialMatrix(matrixSize=self._size, center=(0,0), radius=2)
        tempTex = filters.makeGauss(pin, mean=localf, sd=sigmaF)
        return FT*tempTex
