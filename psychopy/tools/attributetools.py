...
class attributeSetter(object):
    ...
    def __set__(self, obj, value):
        ...
        logAttrib(obj, log=None, attrib=self.func.__name__,
                  value=value)  # log=None defaults to obj.autoLog
        ...
        #print('%s.%s = %s (%s in %s, line %i' %(obj.__class__.__name__, self.func.__name__, value.__repr__(), origin[1], origin[0].split('/')[-1], origin[1], origin[3].__repr__())))  # long
        """
        return newValue
...
def setAttribute(self, attrib, value, log, operation=False, stealth=False):
    ...
    if operation is not False:
        try:
            ...
            # attribute is not set yet. Set it to None to skip operation and
            # just set value.
            oldValue = None
            value = value

        # Apply operation except for the case when new or old value are None or
        # string-like
        if value is not None and not isinstance(value, basestring) and \
            ...
            if operation in ('', None):
                ...
                raise ValueError('Unsupported value "', operation,
                                 '" for operation when setting', attrib, 'in', self.__class__.__name__)

        elif operation not in ('', None):
            raise TypeError('operation %s invalid for %s (old value) and %s (operation value)' % (
                operation.__repr__(), oldValue, value))
