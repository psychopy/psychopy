...
class ConfigWizard(object):
    ...
    def __init__(self, firstrun=False, interactive=True, log=True):
        ...
        if firstrun:
            ...
            dlg.addText(_translate(
                "It looks like you are running PsychoPy for the first time."))
            dlg.addText(_translate(
                "This wizard will help you get started quickly and smoothly."))
        else:
        ...
        if not driversOkay():
            ...
            dlg.addText('')
            dlg.addText(_translate(
                "The first configuration check is your video card's drivers. The current"), color='red')
            dlg.addText(_translate(
                "drivers cannot support PsychoPy, so you'll need to update the drivers."), color='red')
        ...
        if numWarn == 0:
            ...
            msg = _translate(
                '1 suboptimal value was detected (%s)') % self.warnings[0]
        else:
    ...
    def runDiagnostics(self, win, verbose=False):
        ...
        items = info.RunTimeInfo(
            win=win, refreshTest='grating', verbose=True, userProcsDetailed=True)

        totalRAM, freeRAM = items[
            'systemMemTotalRAM'], items['systemMemFreeRAM']
        warn = False
        if freeRAM == 'unknown':
            if totalRAM != 'unknown':
                totalRAM = "%.1fG" % (totalRAM / 1024.)
            msg = _translate(
                'could not assess available physical RAM; total %s') % totalRAM
            report.append(('available memory', 'unknown', msg, warn))
        else:
            msg = _translate('physical RAM available for configuration test (of %.1fG total)') % (
                totalRAM / 1024.)
            if freeRAM < 300:  # in M
                ...
                warn = True
            report.append(
                ('available memory', unicode(freeRAM) + 'M', msg, warn))

        # ----- PSYCHOPY: -----
        warn = False
        report.append(('PsychoPy', '', '', False))  # not localized
        report.append(('psychopy', __version__, _translate(
            'avoid upgrading during an experiment'), False))
        report.append(('locale', items['systemLocale'],
                       _translate(
                           'can be set in <a href="http://www.psychopy.org/general/prefs.html#application-settings-app">Preferences -> App</a>'),
                       False))
        msg = ''
        if items['pythonVersion'] < '2.5' or items['pythonVersion'] >= '3':
            msg = _translate(
                'Warning: python 2.6 or 2.7 required; 2.5 is not supported but might work')
            warn = True
        ...
        report.append(
            ('python version', items['pythonVersion'] + ' &nbsp;(%s)' % bits, msg, warn))
        warn = False
        if verbose:
            msg = ''
            ...
            report.append(('rush', str(items['psychopyHaveExtRush']), _translate(
                'for high-priority threads'), False))
        ...
        if nDropped:
            ...
            warn = True
        report.append(('no dropped frames', '%i / %i' %
                       (nDropped, nTotal), msg, warn))
        ...
        if verbose:
            report.append(('openGL max vertices', str(
                items['openGLmaxVerticesInVertexArray']), '', False))
        ...
        report.append(('numpy', items['pythonNumpyVersion'], _translate(
            'vector-based (fast) calculations'), False))
        report.append(('scipy', items['pythonScipyVersion'], _translate(
            'scientific / numerical'), False))
        report.append(('matplotlib', items['pythonMatplotlibVersion'], _translate(
            'plotting; fast contains(), overlaps()'), False))

        # ----- SYSTEM: -----
        report.append(('System', '', '', False))
        report.append(('platform', items['systemPlatform'], '', False))
        msg = _translate(
            'for online help, usage statistics, software updates, and google-speech')
        warn = False
        ...
        report.append(('auto proxy', str(self.prefs.connections['autoProxy']), _translate(
            'try to auto-detect a proxy if needed; see <a href="http://www.psychopy.org/general/prefs.html#connection-settings-connections">Preferences -> Connections</a>'), False))
        ...
        report.append(('proxy setting', prx, _translate(
            'current manual proxy setting from <a href="http://www.psychopy.org/general/prefs.html#connection-settings-connections">Preferences -> Connections</a>'), False))
        ...
        '''
        report.append(('CPU speed test', "%.3f s" % items['systemTimeNumpySD1000000_sec'], _translate(
            'numpy.std() of 1,000,000 data points'), False))
        ...
        if verbose:  # always False for a real first-run
            ...
            for pkg in packages:
                try:
                    ...
                    report.append(
                        (pkg, '&nbsp;&nbsp;--', _translate('could not import package %s') % pkg, False))
    ...
    def htmlReport(self, items=None, fatal=False):
        ...
        if fatal:
            ...
            for item in fatal:
                item = item.replace(
                    'Critical issue', '<p><strong>' + _translate('Critical issue') + '</strong>')
                htmlDoc += item + "<hr>"
        else:
...
class BenchmarkWizard(ConfigWizard):
    ...
    def __init__(self, fullscr=True, interactive=True, log=True):
        ...
        for shape in ['circle', 'square']:  # order matters: circle crashes first
            dotsList = self.runLotsOfDots(
                win, fieldShape=shape, starting=start, baseline=fps)
        ...
        self.htmlReport(itemsList)
        self.reportPath = os.path.join(
            self.prefs.paths['userPrefsDir'], 'benchmarkReport.html')
        self.save()
        dlg = gui.Dlg(title=self.name)
        dlg.addText('')
        dlg.addText(_translate(
