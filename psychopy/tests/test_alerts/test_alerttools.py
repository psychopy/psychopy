...
class TestAlertTools(object):
    ...
    def test_sizing_x_dimension(self):
        ...
        sys.stderr.flush()
        assert ('Your stimuli size exceeds the X dimension of your window' in alertLog.current[-1].msg)

    def test_sizing_y_dimension(self):
        self.polygonComp.params['size'].val = [.5, 4]
        alerttools.runTest(self.polygonComp)
        sys.stderr.flush()
        assert ('Your stimuli size exceeds the Y dimension of your window' in alertLog.current[-1].msg)

    def test_size_too_small_x(self):
        self.polygonComp.params['size'].val = [.0000001, .05]
        alerttools.runTest(self.polygonComp)
        sys.stderr.flush()
        assert ('Your stimuli size is smaller than 1 pixel (X dimension),' in alertLog.current[-1].msg)

    def test_size_too_small_y(self):
        self.polygonComp.params['size'].val = [.05, .0000001]
        alerttools.runTest(self.polygonComp)
        sys.stderr.flush()
        assert ('Your stimuli size is smaller than 1 pixel (Y dimension),' in alertLog.current[-1].msg)

    def test_position_x_dimension(self):
        self.polygonComp.params['pos'].val = [4, .5]
        alerttools.runTest(self.polygonComp)
        sys.stderr.flush()
        assert ('Your stimuli position exceeds the X dimension' in alertLog.current[-1].msg)

    def test_position_y_dimension(self):
        self.polygonComp.params['pos'].val = [.5, 4]
        alerttools.runTest(self.polygonComp)
        sys.stderr.flush()
        assert ('Your stimuli position exceeds the Y dimension' in alertLog.current[-1].msg)
    ...
    def test_timing(self):
        ...
        alerttools.runTest(self.polygonComp)
        sys.stderr.flush()
        assert ('Your stimuli start time exceeds the stop time' in alertLog.current[-1].msg)

    def test_disabled(self):
        self.polygonComp.params['disabled'].val = True
        alerttools.testDisabled(self.polygonComp)
        sys.stderr.flush()
        assert ('Your component is currently disabled' in alertLog.current[-1].msg)

    def test_python_syntax(self):
        alerttools.checkPythonSyntax(self.codeComp, 'Begin Experiment')
        sys.stderr.flush()
        assert ("Python Syntax Error in 'Begin Experiment'" in alertLog.current[-1].msg)

    def test_javascript_syntax(self):
        alerttools.checkJavaScriptSyntax(self.codeComp, 'Begin JS Experiment')
        sys.stderr.flush()
        assert ("JavaScript Syntax Error in 'Begin JS Experiment'" in alertLog.current[-1].msg)
