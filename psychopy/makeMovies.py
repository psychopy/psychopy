...
def Getalphaindex(imgP, maskinv):
    ...
    # This will be the color to which transparency will be set when saving the
    # file

    xsize, ysize = imgP.size
    # get counts for all colors having non-active alphas
    hist = imgP.histogram(maskinv)
...
def rgb2palette(imgRgb, palette=None, verbose=False):     # image could be a "RGBA"
    ...
    hasalpha = False
    if imgRgb.mode == 'RGBA':
        # for post=processing to create transparency
        hasalpha = True
        if verbose:
        ...
        # = True on active alphas
        mask = source[A].point(lambda i: i < 255 and 255)
        # = True on inactive alphas
        maskinv = source[A].point(lambda i: i == 255)
    # end if

    # find the most popular colors, limiting the max number to 256
    # any "excess" colors with be transformed later to the closest palette
    # match
    if palette is None:
    ...
    if datalist is None:
        # xsize*ysize list of color 3-tuples
    ...
    for yord in xrange(ysize):
        for xord in xrange(xsize):
            # org image color tuple
            pxlcolor = list(datalist[yord * xsize + xord])
    ...
    if hasalpha:
        ...
        if verbose:
            print()
            print('Low color-count image:   least used color index, leastcount = %i, %i' %
                  (indexleastused, leastcount))
        # end if
